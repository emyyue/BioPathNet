...
class KnowledgeGraphCompletionBiomed(tasks.KnowledgeGraphCompletion, core.Configurable):
    ...
    def preprocess(self, train_set, valid_set, test_set):
        ...
        self.graph.num_nodes_per_type = torch.bincount(node_type)
    ...
    def _strict_negative(self, pos_h_index, pos_t_index, pos_r_index):
        if self.conditional_probability:
            ...
            t_mask[pos_index, t_truth_index] = 0
            #t_mask.scatter_(1, pos_h_index.unsqueeze(-1), 0)
            neg_t_candidate = t_mask.nonzero()[:, 1]
            num_t_candidate = t_mask.sum(dim=-1)
            neg_t_index = functional.variadic_sample(neg_t_candidate, num_t_candidate, self.num_negative).squeeze(-1)
